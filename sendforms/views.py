...
def enviar_email_transito(request, pk):
    ...
    email = EmailMessage(
        	...
        	", número: " + str(enviaremail.endereco.numero) +
...
def enviar_email_educacao(request, pk):
    ...
    enviaremail = EnviarEmailEducacao.objects.create(
                                                    ...
                                                    escola = solicitacao_educacao.escola
                                                    )
...
def enviar_email_meioambiente(request, pk):
    ...
    usuario = request.user
    enviaremail = EnviarEmailMeioAmbiente.objects.create(nome = usuario.nome, email = usuario.email, endereco = solicitacao_meioambiente.endereco) 
    if not enviaremail.endereco.complemento:
        logger.info("Sem complemento.")   
    email = EmailMessage(
